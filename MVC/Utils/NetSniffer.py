...
class NetSniffer:
	...
	def run_tshark(self):
		...
		if self.capture_process is None:				#if no capture_process is running...
			...
			try:
				self.capture_process = subprocess.Popen(["tshark", "-i", "any", "-f", f"tcp port {self.port}", "-w", f"generated_traffic_{datetime.now().strftime("%Y-%m-%d_%H:%M:%S")}.pcap"], preexec_fn=os.setsid)
